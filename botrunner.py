...
# ─────────────────────────────────────────────
BOT_TOKEN = os.environ["BOT_TOKEN"]  # ← YAHI CHANGE HAI
ADMIN_IDS = set(
    int(x.strip())
    for x in os.environ.get("ADMIN_IDS", "").split(",")  # ← YAHI CHANGE HAI
    if x.strip()
)
...
# ... BAKI SARI CODE WAISI HI RAHEGI (kuch change mat karna) ...
...
async def handle_document(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # ── ZIP — run EVERY .py and .c file inside ──
    if ext == ".zip":
        ...
        for py_file in py_files:
            ...
            log.info(f"▶️ Running py: {py_file.name} as '{sub_name}'")

        # All .c files
        py_stems = {pf.stem for pf in py_files}   # e.g. {"bot", "main"}
        ...
        for c_file in c_files:
            ...
            # Same stem as a .py file? → helper exe, sirf compile karo, run nahi
            # (bot.py khud call karega jab chahiye)
            if c_file.stem in py_stems:
                log.info(f"🔗 '{c_file.name}' helper hai '{c_file.stem}.py' ke liye — compiled at {exe}, not auto-run")
                errors.append(f"ℹ️ `{c_file.name}` compiled ✅ (helper — `{c_file.stem}.py` call karega)")
                continue

            # Alag stem → independent process, run karo
            ...
            deployed.append(sub_name)
            log.info(f"▶️ Running c: {c_file.name} as '{sub_name}'")
        ...
        # Build reply keyboard with all deployed bot names
...
def main():
    ...
    # Text handler — ReplyKeyboard button presses
